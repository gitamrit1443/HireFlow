...
export class MeetRoomComponent implements OnInit {
  ...
  joinMeeting() {
    ...
    this.meetSvc.joinMeeting(this.roomCode, this.participantName.trim(), role).subscribe(res => {
      ...
      // Build Daily.co iframe URL. If token is empty, use the public/fallback room URL directly.
      const dailyUrl = res.data.dailyRoomUrl || res.data.meeting?.meetUrl;
      const url = res.data.meetingToken ? `${dailyUrl}?t=${res.data.meetingToken}` : dailyUrl;
