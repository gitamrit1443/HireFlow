...
export class AuthService {
    constructor() {
        ...
        this.currentUser = signal(this._load(), ...(ngDevMode ? [{ debugName: "currentUser" }] : []));
        this.isLoggedIn = computed(() => !!this.currentUser(), ...(ngDevMode ? [{ debugName: "isLoggedIn" }] : []));
        this.isRecruiter = computed(() => this.currentUser()?.role === 'recruiter', ...(ngDevMode ? [{ debugName: "isRecruiter" }] : []));
        this.isCandidate = computed(() => this.currentUser()?.role === 'candidate', ...(ngDevMode ? [{ debugName: "isCandidate" }] : []));
        this.userInitials = computed(() => this.currentUser()?.initials ?? '?', ...(ngDevMode ? [{ debugName: "userInitials" }] : []));
        this.userName = computed(() => this.currentUser()?.name ?? '', ...(ngDevMode ? [{ debugName: "userName" }] : []));
        this.userTitle = computed(() => this.currentUser()?.title ?? '', ...(ngDevMode ? [{ debugName: "userTitle" }] : []));
    }
    ...
    verify2FA(code) {
        ...
        return new Promise(resolve => {
            this.http.post(`${environment.apiUrl}/auth/login/2fa`, { tempToken, code }).subscribe({
                next: res => {
                    if (!res.success) {
                        resolve({ success: false, message: 'Invalid code.' });
                        return;
                    }
                    localStorage.removeItem('hireflow_2fa_token');
                    this._store(res);
                    resolve({ success: true, message: 'Welcome back!' });
                },
    ...
    async register(data) {
        ...
        try {
            const response = await firstValueFrom(this.http.post(`${environment.apiUrl}/auth/register`, payload));
            if (!response?.success) {
    ...
    _store(res) {
        ...
        const user = {
            id: u.id, name, email: u.email, role: roleRaw, avatar: '',
    ...
    _load() {
        try {
            const r = localStorage.getItem('hireflow_user');
