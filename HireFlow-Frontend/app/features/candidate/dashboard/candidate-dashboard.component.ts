...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-slate-50">
      ...
      <header class="bg-white border-b border-slate-200 sticky top-0 z-10">
        <div class="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div class="relative flex items-center gap-3">
            ...
            @if (accountOpen()) {
              <div class="absolute right-0 top-full mt-3 w-52 bg-white rounded-xl border border-slate-200 shadow-lg py-1 z-30">
                ...
                <div class="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-card-hover transition-shadow">
                  ...
                  <div class="flex items-center justify-between gap-3">
                    <button (click)="toggleBookmark(job.id)"
                      class="btn btn-sm btn-ghost border border-slate-200 normal-case">
                      {{ bookmarkedJobs().includes(job.id) ? 'Bookmarked' : 'Bookmark' }}
                    </button>
                    <button (click)="applyToJob(job)" class="btn btn-sm btn-primary normal-case">Apply Now</button>
                  </div>
                </div>
              }
            </div>
      ...
      @if (editOpen()) {
        <div class="fixed inset-0 z-50 bg-slate-900/35 flex items-center justify-center p-4">
...
export class CandidateDashboardComponent {
  ...
  interestTags = signal(['Frontend', 'Angular', 'TypeScript', 'Remote']);
  ...
  readonly recommendedJobs = computed(() =>
    [...this.mockJobs].sort((a, b) => this.matchScore(b) - this.matchScore(a))
  );
