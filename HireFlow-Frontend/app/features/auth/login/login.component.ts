    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-slate-100 to-indigo-50
      ...
      <div class="w-full max-w-sm">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
          ...
          <div class="mb-6 rounded-xl border border-indigo-100 bg-indigo-50 p-3.5">
            <p class="text-xs font-bold text-indigo-700 mb-2">🔑 Demo Credentials</p>
            <div class="grid grid-cols-2 gap-2">
              <button type="button"
                (click)="fillDemo('recruiter')"
                class="text-left p-2 rounded-lg bg-white border border-indigo-200
                       hover:border-indigo-400 hover:bg-indigo-50 transition-all cursor-pointer">
                <p class="text-xs font-bold text-indigo-700">👔 Recruiter</p>
                <p class="text-xs text-indigo-500 mt-0.5 truncate">recruiter&#64;hireflow.com</p>
              </button>
              <button type="button"
                (click)="fillDemo('candidate')"
                class="text-left p-2 rounded-lg bg-white border border-indigo-200
                       hover:border-indigo-400 hover:bg-indigo-50 transition-all cursor-pointer">
                <p class="text-xs font-bold text-indigo-700">🙋 Candidate</p>
                <p class="text-xs text-indigo-500 mt-0.5 truncate">candidate&#64;hireflow.com</p>
              </button>
            </div>
            <p class="text-xs text-indigo-500 mt-2 text-center">Password: <strong>demo1234</strong></p>
          </div>
...
export class LoginComponent {
  ...
  fillDemo(role: 'recruiter' | 'candidate'): void {
    const email = role === 'recruiter' ? 'recruiter@hireflow.com' : 'candidate@hireflow.com';
    this.loginForm.setValue({ email, password: 'demo1234' });
