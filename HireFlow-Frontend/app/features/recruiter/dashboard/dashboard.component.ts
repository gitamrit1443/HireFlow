  ...
  template: `
    <div class="p-6 space-y-6 max-w-screen-2xl mx-auto">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 class="text-xl font-bold text-slate-900">Good morning, Rahul 👋</h1>
...
export class RecruiterDashboardComponent {
  ...
  readonly stats              = signal<StatCard[]>(this.dashboardService.getStatCards());
  readonly pipelineStages     = signal<PipelineStageCount[]>(this.dashboardService.getPipelineStages());
  readonly recentActivity     = signal<ActivityItem[]>(this.dashboardService.getRecentActivity());
  readonly upcomingInterviews = signal<UpcomingInterview[]>(this.dashboardService.getUpcomingInterviews());
  readonly recentCandidates   = signal<Candidate[]>(this.mockData.candidates.slice(0, 7));

  readonly totalCandidates = signal(this.mockData.candidates.filter(c => c.stage !== 'rejected').length);
  readonly selectedCount   = signal(this.mockData.candidates.filter(c => c.stage === 'selected').length);
  readonly rejectedCount   = signal(this.mockData.candidates.filter(c => c.stage === 'rejected').length);
  readonly conversionRate  = signal(
    Math.round(
      (this.mockData.candidates.filter(c => c.stage === 'selected').length /
       this.mockData.candidates.length) * 100
    )
  );
