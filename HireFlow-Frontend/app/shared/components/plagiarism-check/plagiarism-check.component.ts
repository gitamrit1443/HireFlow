...
export class PlagiarismCheckComponent implements OnInit {
  ...
  runCheck() {
    ...
    this.http.post<{ success: boolean; data: AnalysisResult }>(
      ...
      catchError(() => of({ success: false, data: this._mockResult() }))
    ).subscribe(res => {
  ...
  private _mockResult(): AnalysisResult {
    return {
      applicationId: this.applicationId,
      candidateName: this.candidateName,
      plagiarism: {
        similarityPercent: 12, isPlagiarised: false,
        riskLevel: 'low', matchedSegments: [],
        verdict: '✅ Original. No significant matches found.'
      },
      spam: {
        isSpam: false, spamPercent: 8,
        riskLevel: 'low', flags: [],
        verdict: '✅ Appears legitimate.'
      },
      overallRisk: 'low',
      recommendation: 'clear',
      summary: `✅ ${this.candidateName}'s application looks clean. (Demo mode — connect backend for real analysis)`
    };
  }
}
