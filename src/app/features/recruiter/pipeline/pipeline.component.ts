        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          ...
          <div class="flex items-center gap-2">
            <select [(ngModel)]="jobFilter"
              ...
              @for (job of mockData.jobs; track job.id) {
...
export class PipelineComponent {
  ...
  private _candidates = signal<Candidate[]>([...this.mockData.candidates]);
  ...
  moveCandidate(id: string, newStage: HiringStage | 'rejected'): void {
    const c = this._candidates().find(x => x.id === id);
    if (!c || c.stage === newStage) return;
    this._candidates.update(list =>
      list.map(x => x.id === id ? { ...x, stage: newStage as HiringStage } : x)
    );
  ...
  moveFromPanel(stage: HiringStage | 'rejected'): void {
    const c = this.selectedCandidate();
    if (c) this.moveCandidate(c.id, stage);
  }
  ...
  refreshBoard(): void {
    this._candidates.set([...this.mockData.candidates]);
